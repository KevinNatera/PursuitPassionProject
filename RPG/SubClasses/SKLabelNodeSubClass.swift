//
//  SKLabelNodeSubClass.swift
//  RPG
//
//  Created by Kevin Natera on 11/20/19.
...
extension SKAdvancedLabelNode {
    ...
    func sequentiallyBouncingZoom(delay:TimeInterval, infinite:Bool ) {
        if labels.count > 0 && self.action(forKey: "sequentiallyBouncingZoom") == nil {
            let main = SKAction.run { [weak self] in
                ...
                for i in 0..<strongSelf.labels.count {
                    let zoomIn = SKAction.scale(to: 8.0, duration: delay)
    ...
    func shake(delay:TimeInterval, infinite:Bool = false, duration:TimeInterval = 5.0, amplitudeX:CGFloat = 0.0, amplitudeY:CGFloat = 40.0, speed:TimeInterval=0.04) {
